...
if ($action == "uploadimage") {
  ...
  if ((empty($_FILES['media_file']['tmp_name']) || $_FILES['media_file']['tmp_name'] == "none") && ($remote_media_file == "" || !check_remote_media($remote_media_file))) {
    $error = 1;
  ...
  if (!$error) {
    ...
    // Upload Media file
    if (!empty($_FILES['media_file']['tmp_name']) && $_FILES['media_file']['tmp_name'] != "none") {
    ...
    elseif ($config['auto_thumbnail'] == 1 && !empty($new_name) && !$uploaderror && ((!empty($_FILES['media_file']['tmp_name']) && $_FILES['media_file']['tmp_name'] != "none") || is_remote($new_name))) {
      if ($direct_upload) {
